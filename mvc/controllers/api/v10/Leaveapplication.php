...
class Leaveapplication extends Api_Controller 
{
    ...
    public function index_get() 
    {
        ...
        if($usertypeID != 1) {
            $this->retdata['leaveapplications'] = $this->leaveapplication_m->get_order_by_leaveapplication(['applicationto_userID' => $userID, 'applicationto_usertypeID' => $usertypeID, 'schoolyearID' => $schoolyearID]);
        } else {
            $this->retdata['leaveapplications'] = $this->leaveapplication_m->get_order_by_leaveapplication(array('schoolyearID' => $schoolyearID));
        }
        $this->retdata['leavecategorys'] = pluck($this->leavecategory_m->get_leavecategory(), 'leavecategory', 'leavecategoryID');
        $this->retdata['allUser'] = getAllUserObjectWithStudentRelationNameForApi($schoolyearID);
